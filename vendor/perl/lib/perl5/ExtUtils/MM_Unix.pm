...
Makefile entry for a _exists file in these directories. Returns
...
# dir_target(@array) returns a Makefile entry for the file _exists in each
...
$(BOOTSTRAP): '."$self->{MAKEFILE} $self->{BOOTDEP}".' $(INST_ARCHAUTODIR)/_exists
...
$(INST_BOOT): $(BOOTSTRAP) $(INST_ARCHAUTODIR)/_exists
...
$(INST_DYNAMIC): $(OBJECT) $(MYEXTLIB) $(BOOTSTRAP) $(INST_ARCHAUTODIR)/_exists $(EXPORT_LIST) $(PERL_ARCHIVE) $(PERL_ARCHIVE_AFTER) $(INST_DYNAMIC_DEP)
');
    if ($armaybe ne ':'){
	$ldfrom = 'tmp$(LIB_EXT)';
...
pure_perl_install ::
		...
		read }.$self->catfile('$(PERL_ARCHLIB)','auto','$(FULLEXT)','_packlist').q{ \
		write }.$self->catfile('$(INSTALLARCHLIB)','auto','$(FULLEXT)','_packlist').q{ \
...
pure_site_install ::
		...
		read }.$self->catfile('$(SITEARCHEXP)','auto','$(FULLEXT)','_packlist').q{ \
		write }.$self->catfile('$(INSTALLSITEARCH)','auto','$(FULLEXT)','_packlist').q{ \
...
uninstall_from_perldirs ::
	...
	q{$(UNINSTALL) }.$self->catfile('$(PERL_ARCHLIB)','auto','$(FULLEXT)','_packlist').q{

uninstall_from_sitedirs ::
	}.$self->{NOECHO}.
	q{$(UNINSTALL) }.$self->catfile('$(SITEARCHEXP)','auto','$(FULLEXT)','_packlist').q{
};
...
realclean ::
	...
	my $todir = dirname($to);
	push @m, "
$to: $from $self->{MAKEFILE} " . $self->catdir($todir,'_exists') . "
...
";
    ...
    push @m, "
\$(INST_ARCHAUTODIR)/extralibs.all: \$(INST_ARCHAUTODIR)/_exists ".join(" \\\n\t", @$extra)."
...
#    return '' unless $self->needs_linking(); #might be because of a subdir
    ...
    my(@m);
    push(@m, <<'END');
$(INST_STATIC): $(OBJECT) $(MYEXTLIB) $(INST_ARCHAUTODIR)/_exists
	$(RM_RF) $@
END
...
# This helps us to minimize the effect of the _exists files A yet
...
config :: '.$self->{MAKEFILE}.' $(INST_LIBDIR)/_exists
	'.$self->{NOECHO}.'$(NOOP)

config :: $(INST_ARCHAUTODIR)/_exists
	'.$self->{NOECHO}.'$(NOOP)

config :: $(INST_AUTODIR)/_exists
	'.$self->{NOECHO}.'$(NOOP)
';
    ...
    if (%{$self->{HTMLLIBPODS}}) {
	push @m, qq[
config :: \$(INST_HTMLLIBDIR)/_exists
	$self->{NOECHO}\$(NOOP)
	...
	push @m, qq[
config :: \$(INST_HTMLSCRIPTDIR)/_exists
	$self->{NOECHO}\$(NOOP)
