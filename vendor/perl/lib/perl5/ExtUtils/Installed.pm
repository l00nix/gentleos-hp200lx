...
$self->{Perl}{packlist} =
   ExtUtils::Packlist->new("$Config{installarchlib}/_packlist");
$self->{Perl}{version} = $Config{version};

# Read the module packlists
my $sub = sub
   {
   # Only process module _packlists
   return if ($_) ne "_packlist" || $File::Find::dir eq $Config{installarchlib};

   # Hack of the leading bits of the paths & convert to a module name
   my $module = $File::Find::name;
   $module =~ s!$Config{archlib}/auto/(.*)/_packlist!$1!s;
   $module =~ s!$Config{sitearch}/auto/(.*)/_packlist!$1!s;
...
files have been installed.  It uses the information stored in _packlist files
...
information from the _packlist files.

=head1 USAGE

The new() function searches for all the installed _packlists on the system, and
stores their contents. The _packlists can be queried with the functions
described below.
...
This takes no parameters, and searches for all the installed _packlists on the
...
the files listed in the modules _packlist actually exist, and returns a list of
any missing files.  If an optional second argument which evaluates to true is
given any missing files will be removed from the _packlist
