...
sub install {
	...
	for (readdir DIR) {
	    next if $_ eq "." || $_ eq ".." || $_ eq "_exists";
	...
	#timestamp and permission and remember for the _packlist
...
sub install_default {
  ...
  install({
	   read => "$Config{sitearchexp}/auto/$FULLEXT/_packlist",
	   write => "$Config{installsitearch}/auto/$FULLEXT/_packlist",
